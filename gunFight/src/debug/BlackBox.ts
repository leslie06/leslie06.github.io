...
interface Sample {
  ...
  ms: number;
  scale: number;
  wave: number; kills: number; enemies: number;
  dom: number;
}
...
const KEY = 'gunfight.blackbox';
const LAST_KEY = 'gunfight.blackbox.last';
const INTERVAL = 5;
const KEEP = 48; // 4 minutes at 5 s

interface GameLike { wave?: number; kills?: number }
interface EnemiesLike { aliveCount?: () => number }
...
export class BlackBox {
  ...
  update(dt: number): void {
    ...
    const game = e.get<GameLike & { name: string }>('game');
    const enemies = e.get<EnemiesLike & { name: string }>('enemies');
    this.rec.samples.push({
      ...
      wave: game?.wave ?? 0, kills: game?.kills ?? 0, enemies: enemies?.aliveCount?.() ?? 0, dom: document.getElementsByTagName('*').length,
    });
  ...
  summary(): string {
    ...
    return `上次异常结束 @${fmtT(last.t)}  堆 ${first.heap}→${last.heap}MB  纹理 ${first.tex}→${last.tex}  几何 ${first.geo}→${last.geo}  帧 ${last.ms}ms  波 ${last.wave}`;
  }

  /** The crashed record as text for the player to paste back. */
  report(): string {
    const c = this.crashed; if (!c) return '';
    const head = `GUNFIGHT 上次会话在没有正常关闭的情况下结束（浏览器重载/杀掉了页面）\n开始 ${c.started}  导航 ${c.nav}  档位 ${c.tier}  缓冲 ${c.buffer}\nGPU ${c.gpu}\n${c.ua}\n`;
    const cols = ['t', 'heap', 'tex', 'geo', 'prog', 'calls', 'ms', 'scale', 'wave', 'kills', 'enemies', 'dom'] as const;
...
function pickStorage(): Storage | null {
  try { localStorage.setItem('gunfight.probe', '1'); localStorage.removeItem('gunfight.probe'); return localStorage; } catch { /* fall through */ }
  try { sessionStorage.setItem('gunfight.probe', '1'); sessionStorage.removeItem('gunfight.probe'); return sessionStorage; } catch { return null; }
}
